...
import { RegionData, nationalCategories, categoryColorClasses } from "@/data/mockData";
...
export function MapSidePanel({ selectedRegion }: MapSidePanelProps) {
  if (!selectedRegion) {
    // Default: national overview with bar chart
    const maxVal = Math.max(...Object.values(nationalCategories));
    ...
    return (
      <div className="flex h-full flex-col rounded-xl border bg-card p-5 shadow-sm md:p-6">
        ...
        <div className="flex flex-1 flex-col justify-center gap-4">
          {Object.entries(nationalCategories).map(([cat, count]) => {
            ...
            return (
              <div key={cat} className="space-y-1.5">
                ...
                <div className="h-2.5 w-full overflow-hidden rounded-full bg-muted">
                  <div
                    className={`h-full rounded-full ${categoryColorClasses[cat]} transition-all duration-700`}
  ...
  const totalPrograms = selectedRegion.programCount;
  ...
  return (
    <div className="flex h-full flex-col rounded-xl border bg-card p-5 shadow-sm md:p-6">
      ...
      <div className="mb-5 flex items-baseline gap-1">
        <span className="text-3xl font-bold text-primary">{totalPrograms}</span>
      ...
      <Link to={`/region/${selectedRegion.id}`} className="mt-5">
